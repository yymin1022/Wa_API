...
def message_friends(message, room, sends):
    if "동훈" in message:
        ...
        if "바보" in message:
            return message_stupid_yongmin(0)
        elif "천재" in message:
            return message_stupid_yongmin(1)
        else:
            return message_stupid_yongmin(2)
    elif "용민" in message:
...
def message_joohyeong():
    str_message = "예! 2025년도 CECOM 회장 이주형!"

    return str_message

def message_junseob():
    messages = ["준섭아 컴공인 척 하지마",
                "준섭이는 유명한 납땜러",
                "준섭아 이상한거 좀 그만 사...",
                "준섭이가 사주는 술 마시러 갈사람~",
                "김준섭 박사기원 N일차",
...
def message_minseok():
    str_message = "와봇은 민석이가 지배했다!"

    return str_message

def message_minsik():
    str_message = "민식아 그래서 학교는 언제와?"
...
def message_soohyun():
    str_message = "수현이? 무슨 수현이?"
    return str_message

def message_stupid_yongmin(stupid_type):
    str_message = ""
    if stupid_type == 0:
        str_message = "그렇지~"
    elif stupid_type == 1:
...
def message_gdg():
    str_message = "GDG on Campus: CAU 최고 ~!~!~!~!@"
    return str_message

def message_not_gdsc():
    str_message = ["아뇨. GDG 인데요.",
                   "이제 GDG라니깐요?!",
                   "GDG입니다.",
                   "GDSC는 이제 없어요.",
                   "GDG! GDG!! GDG!@!@!@!"]
