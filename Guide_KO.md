...
봇 제작에 참여하고자 하시는 경우, ```message.py``` 를 참고해 코드를 작성하시고, ```Pull Request```를 등록해주시면 주기적으로 확인하여 올바른지 검토한 뒤 ```Merge``` 해드립니다!<br/>
단, 전반적인 코드 스타일에 맞추어 올바른 ```message_util``` 디렉토리의 파일에 코드를 작성해주시기 바랍니다.<br/>
